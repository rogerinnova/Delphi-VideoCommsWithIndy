unit IsNextGenPickup;
//shared code posted as unit IsAnsiStringModule
//at https://github.com/rogerinnova/Delphi-NextGen-Compiler-Utilities
//and http://delphinotes.innovasolutions.com.au/posts/ansistring-on-an-android-device
...
uses ISUnicodeStrUtl, IsWindowsPickUp;
...
begin
  ...
  begin
    ...
    Ri := 0;
    Ui := 0;
    PAUCode:=PChar(AUCode);
    CharVal := Ord(PAUCode[0]);
    while (Ri < Finished) and (CharVal < 256) do
    begin
      CharVal := Ord(PAUCode[Ui]);
      FData[Ri] := CharVal;
      Inc(Ui);
      Inc(Ri);
    end;
...
function AnsiString.LastChar: Integer;
begin
  Result := System.Length(FData) - 2;
  if Result < -1 then
    Result := -1;
end;
...
begin
  ...
  EndPos := AStm.seek(LongInt(0), soFromEnd);
  ...
  if CurPos < EndPos then
    AStm.seek(LongInt(CurPos - 1), soFromBeginning);
end;
  ...
  Begin
    if ABytes > 0 then
      if ABytes < ToWrite then
        ToWrite := ABytes;
    //Src := @FData[AOffset];
    Result := AStm.Write(FData[AOffset], ToWrite);
...
begin
  ...
  if Len > 0 then
    for i := 0 to Len - 1 do
      PResult[i] := Char(TISBytesArray(FData)[i]);
end;
...
Var
  Len: integer;
  OffsetPtr,OffsetPtr2:PByte;
  Offset:Integer;
begin
  If FData = Pointer(FTempAnsiString) then
    Offset:=0
    else
      begin
      OffsetPtr:=FData;
      OffsetPtr2:= Pointer(FTempAnsiString);
      Offset:=OffsetPtr-OffsetPtr2;
      end;
  if Offset<0 then
    Result := 0
  else
  ...
  if ABytes < 0 then
    ...
    Len := ABytes;
  if (Len > 0) then
    Result := AStm.Write(FTempAnsiString.FData[Offset], Len)
  else
    Result := 0;
  End;
end;
